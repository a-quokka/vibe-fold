...
| 9 | `thumb.webp` | ✅ 완료 (2차. 1차 `thumb.svg`는 스프라이트 이전의 손그림이라 교체 — [사유](#9-thumbwebp--목록-카드-썸네일)) |
...
**폴더 구조** — 스프라이트는 `assets/` 하위에 둔다. 챕터 루트에는 계약 파일(`index.html`, `meta.yaml`, `thumb.webp`)만 남긴다.

```
ch-seirenui-norae/
  index.html   meta.yaml   thumb.webp   ASSETS.md
...
## 9. `thumb.webp` — 목록 카드 썸네일

초판 썸네일은 스프라이트가 나오기 전에 만든 `thumb.svg`였다. 캔버스가 그리던 도형을 손으로 옮긴 그림이라, 스프라이트가 다 들어온 뒤에는 **본편과 다른 그림**이 목록에 걸려 있는 상태가 됐다. 그래서 같은 스프라이트로 다시 짰다.

**규격** — 594 × 840 (`297:420` 카드의 2배). 카드는 `aspect-ratio: 297/420` · `object-fit: cover`로 잘리고, 실제 표시 폭은 152\~223px이다([ARTWORK.md](../../../ARTWORK.md#thumbnail)).

**구성** — 왼쪽 위에 달, 오른쪽에 바위와 세이렌, 왼쪽 아래 수면에 배와 묶인 오디세우스. 노랫결은 **원이 아니라 호(弧)**로 깔았다 — 사방으로 퍼지는 원을 그리면 작은 크기에서 과녁처럼 읽혀서, 배 쪽으로 가는 방향만 밝히고 등 뒤는 죽였다.

**레이어 순서** — 하늘 → 달 → 배경 → **바위 → 물** → 물마루 → 배 → 세이렌 → 노랫결 → 비네트. 바위를 물보다 먼저 깔아야 밑동이 수면에 잠기고, 나중에 깔면 바닥이 직선으로 끊긴다. 본편 `drawWater()`가 절벽 뒤에 오는 것과 같은 이유다.

**색** — 하늘·바다·금색 모두 [공통 규격](#공통-규격)의 hex 그대로. 달빛만 `#fac468`로 한 단계 밝게 썼다.

**만드는 법** — `assets/`의 스프라이트를 Pillow로 합성한다. 손으로 그린 부분은 달·수면·달빛 길·노랫결·비네트뿐이고 나머지는 전부 기존 스프라이트다. 스프라이트를 다시 뽑으면 썸네일도 같은 자리에서 다시 합성하면 된다.
...
- [ ] 챕터 폴더 합계 300KB 이하
- [x] 썸네일: 세로형, 본편과 같은 스프라이트로 합성, 152px 폭에서 배와 세이렌이 구분될 것
